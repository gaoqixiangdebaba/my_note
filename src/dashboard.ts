...
interface NoteItem { id: number; content: string; tag?: string; created: string; }
interface QuestionItem { id: number; content: string; resolved: boolean; created: string; resolvedAt?: string; }
interface TaskItem { id: number; content: string; done: boolean; created: string; doneAt?: string; }
interface Store {
  notes: NoteItem[];
...
function loadStore(): Store {
  try {
    ...
    if (!fs.existsSync(STORAGE_FILE)) {
      const empty: Store = { notes: [], questions: [], tasks: [], counters: { notes: 0, questions: 0, tasks: 0 } };
      saveStore(empty);
      return empty;
    }
...
function toggleItem(type: "question" | "task", id: number): { success: boolean; message: string } {
  const store = loadStore();
  if (type === "question") {
    const item = store.questions.find((q) => q.id === id);
    if (!item) return { success: false, message: `问题 Q${id} 不存在` };
    ...
    if (!item) return { success: false, message: `任务 T${id} 不存在` };
    item.done = !item.done;
...
function deleteItem(type: "note" | "question" | "task", id: number): { success: boolean; message: string } {
  const store = loadStore();
  let arr: any[];
  let label: string;
  if (type === "note") { arr = store.notes; label = `N${id}`; }
  else if (type === "question") { arr = store.questions; label = `Q${id}`; }
  else { arr = store.tasks; label = `T${id}`; }

  const idx = arr.findIndex((item: any) => item.id === id);
  if (idx === -1) return { success: false, message: `${label} 不存在` };
  arr.splice(idx, 1);
  saveStore(store);
  return { success: true, message: `${label} 已删除` };
}

function addItem(type: "note" | "question" | "task", content: string): { success: boolean; message: string } {
  const store = loadStore();
  const now = nowISO();
  if (type === "note") {
...
export function startDashboardServer(port: number = 3456): void {
  const server = http.createServer((req, res) => {
    ...
    if (deleteMatch && req.method === "DELETE") {
      const type = deleteMatch[1] as "note" | "question" | "task";
