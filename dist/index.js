...
function classifyLine(line) {
    ...
    for (const kw of ["问题", "疑问", "Q:", "Q：", "question"]) {
        if (lower.includes(kw.toLowerCase()) && /[:：]/.test(trimmed))
            return "question";
    }
    // 4. 显式任务标记（带冒号）
...
function detectSection(line) {
    ...
    if (header.includes("待办") ||
        header.includes("任务") ||
        header.includes("task") ||
        header.includes("todo")) {
        return "task";
    }
    if (header.includes("笔记") ||
...
function formatItems(store, type, status) {
    const parts = [];
    ...
    if (type === "questions" || type === "all") {
        ...
        if (status === "pending")
            items = items.filter((q) => !q.resolved);
        if (status === "done")
            items = items.filter((q) => q.resolved);
        if (items.length > 0) {
...
// ==================== MCP Server ====================
const server = new Server({ name: "dev-notes-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
// ----- 工具列表 -----
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "import_text",
            description: "导入一段文本内容，自动解析并分类为笔记、问题和待办任务。" +
        ...
        {
            name: "list_pending",
            description: "列出所有未解决的问题和未完成的待办任务。" +
        ...
        {
            name: "mark_done",
            description: "标记一个问题为已解决，或标记一个待办任务为已完成。" +
        ...
        {
            name: "list_items",
            description: "列出指定类型的所有条目。可查看笔记、问题或任务，" +
        ...
        {
            name: "get_summary",
            description: "获取所有笔记、问题和任务的统计概览，包括总数、已完成数、待处理数，" +
        ...
        {
            name: "clear_all",
            description: "清空所有存储的数据（笔记、问题、任务全部删除）。" +
    ...
    try {
        switch (name) {
            ...
            case "import_text": {
                ...
                if (!text || !text.trim()) {
                    return {
                        content: [{ type: "text", text: "错误：文本内容为空" }],
                        isError: true,
                    };
                }
            ...
            case "list_pending": {
                const store = loadStore();
                return {
                    content: [{ type: "text", text: formatPending(store) }],
                };
            }
            // ============ mark_done ============
            case "mark_done": {
                ...
                if (!type || (type !== "question" && type !== "task")) {
                    return {
                        content: [
                            { type: "text", text: "错误：type 必须是 'question' 或 'task'" },
                        ],
                        isError: true,
                    };
                }
                if (typeof id !== "number") {
                    return {
                        content: [{ type: "text", text: "错误：id 必须是数字" }],
                        isError: true,
                    };
                }
                const store = loadStore();
            ...
            case "list_items": {
                ...
                return {
                    content: [
                        { type: "text", text: formatItems(store, type, status) },
                    ],
                };
            }
            // ============ get_summary ============
            case "get_summary": {
                const store = loadStore();
                return {
                    content: [{ type: "text", text: formatSummary(store) }],
                };
            }
            // ============ add_item ============
            case "add_item": {
                ...
                if (!type || (type !== "note" && type !== "question" && type !== "task")) {
                    return {
                        content: [
                            { type: "text", text: "错误：type 必须是 'note'、'question' 或 'task'" },
                        ],
                        isError: true,
                    };
                }
